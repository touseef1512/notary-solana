...
function getCachePath(ticker: string, type: 'dividends' | 'splits') {
  return path.join(CACHE_DIR, `${ticker}-${type}.json`);
}

async function getCachedData<T>(ticker: string, type: 'dividends' | 'splits'): Promise<T | null> {
...
function setCachedData<T>(ticker: string, type: 'dividends' | 'splits', data: T) {
  const cachePath = getCachePath(ticker, type);
  try {
...
async function fetchFromAlphaVantage(functionName: 'DIVIDENDS' | 'SPLITS', ticker: string) {
  ...
  let url = `https://www.alphavantage.co/query?function=${functionName}&symbol=${ticker}&apikey=${apiKey || 'demo'}`;
...
export async function getDividendHistory(ticker: string): Promise<DividendRecord[]> {
  ...
  const formattedRecords: DividendRecord[] = records.map((record: any) => ({
...
export async function getSplitHistory(ticker: string): Promise<SplitRecord[]> {
  ...
  const formattedRecords: SplitRecord[] = records.map((record: any) => ({
...
export async function getStockPrice(ticker: string, date?: string): Promise<{ price: number, date: string } | null> {
  ...
  const cached = await getCachedData<{ price: number, date: string }>(ticker, cacheType as any);
  if (cached) return cached;
  ...
  setCachedData(ticker, cacheType as any, result);
  return result;
}
